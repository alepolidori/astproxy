...
(function() {
  ...
  try {
    ...
    /**
     * Command plugin to get the call forward to voicemail on unavailable status of an extension.
     *
     * Use it with _astproxy_ module as follow:
     *
     *     astproxy.doCmd({ command: 'cfuVmGet', exten: '214' }, function (res) {
     *         // some code
     *     });
     *
     * @class cfuVmGet
    ...
    var cfuVmGet = {
      ...
      execute: function(am, args, cb) {
        try {
          ...
          act.ActionID = action.getActionId('cfuVmGet_' + args.exten);
      ...
      data: function(data) {
        try {
          ...
          if (map[data.actionid] && data.event === 'DBGetResponse' &&
            ...
            // to voicemail use the same key database: CFU, but the second adds
            // a prefix to it
            var vm;
            var pre;
            var isCf2Vm = false;
    ...
    // public interface
    exports.data = cfuVmGet.data;
    exports.execute = cfuVmGet.execute;
    exports.setLogger = cfuVmGet.setLogger;
